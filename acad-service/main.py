...
async def get_IPS():
    try:
        with get_db_connection() as data:
            ...
            query = "select m.nim, m.nama, m.jurusan, krs.nilai, mk.sks  from mahasiswa m join krs on krs.nim = m.nim join mata_kuliah mk ON mk.kode_mk = krs.kode_mk where m.nim = '22001'"
            ...
            if not rows:
                raise HTTPException(
                    status_code=404, detail="Data mahasiswa tidak ditemukan"
                )
            ...
            for i in rows:
                value_in_angka = nilai_map.get(i["nilai"].strip(), 0)
                sks = i["sks"]
